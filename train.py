...
# --- Configuration ---
# Data Paths
STOCK_PRICES_PATH = 'stock_prices_30.csv'
...
parser.add_argument('--wandb_project', type=str, default="stock_prediction_tmd", help='WandB project name')
...
class TMD_Transformer(nn.Module):
    ...
    def __init__(self, num_financial_features, num_tickers, ticker_embedding_dim,
        ...
        if self.combined_daily_dim_before_proj % transformer_nhead != 0:
            ...
            # print(f"Projecting combined_daily_dim from {self.combined_daily_dim_before_proj} to {self.d_model}")
        else:
    ...
    def forward(self, financial_data, news_input_ids, news_attention_mask, ticker_ids):
        ...
        title_embeddings_flat = text_outputs.last_hidden_state[:, 0, :] 
...
if __name__ == '__main__':
    ...
    model = TMD_Transformer(
        ...
        ticker_embedding_dim=TICKER_EMBEDDING_DIM, text_model_name=TEXT_MODEL_NAME,
    ...
    if args.load_model_path:
        ...
        try:
            # Adjust for DataParallel if model was saved from it
            state_dict = torch.load(args.load_model_path, map_location=DEVICE)
            # Create a new state_dict if the saved one has 'module.' prefix (from DataParallel)
            # and the current model is not DataParallel (or vice-versa)
            if isinstance(model, nn.DataParallel) and not all(k.startswith('module.') for k in state_dict.keys()):
                # Current model is DP, saved is not: add 'module.'
                new_state_dict = {'module.' + k: v for k, v in state_dict.items()}
                model.load_state_dict(new_state_dict)
            elif not isinstance(model, nn.DataParallel) and all(k.startswith('module.') for k in state_dict.keys()):
                # Current model is not DP, saved is: remove 'module.'
                new_state_dict = {k.replace('module.', '', 1): v for k, v in state_dict.items()}
                model.load_state_dict(new_state_dict)
            else: # Both DP or both not DP, or one is DP and saved is already compatible
    ...
    if DEVICE.type == 'cuda' and num_gpus > 1 and not isinstance(model, nn.DataParallel): # Wrap if not already wrapped (e.g. when loading non-DP model)
    ...
    scaler = GradScaler(enabled=(DEVICE.type == 'cuda')) # Use torch.amp.GradScaler
    ...
    if args.eval_only:
        ...
        if val_loader:
            ...
            eval_loss = evaluate_epoch(model, val_loader, criterion, DEVICE, epoch_num=0) # epoch_num=0 for eval
    ...
    for epoch in range(1, current_epochs + 1): # Start epochs from 1 for logging
        ...
        checkpoint_name = f"tmd_transformer_epoch_{epoch}.pth"
        state_to_save = model.module.state_dict() if isinstance(model, nn.DataParallel) else model.state_dict()
        torch.save(state_to_save, checkpoint_name)
        # wandb.save(checkpoint_name) # Save to wandb artifacts
        ...
        if val_loader and not math.isnan(current_val_loss) and current_val_loss < best_val_loss:
            ...
            best_model_path = "tmd_transformer_best_model.pth"
            torch.save(state_to_save, best_model_path)
            # wandb.save(best_model_path) # Save best model to wandb artifacts
            print(f"Saved new best model with Val Loss: {best_val_loss:.6f}")
            wandb.summary["best_val_loss"] = best_val_loss # Update summary for best
